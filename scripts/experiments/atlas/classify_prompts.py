...
BUNDLES = os.path.join(FP_DIR, 'data', 'bundles')
...
def main():
    ...
    os.makedirs(os.path.join(HERE, 'data', 'prompts'), exist_ok=True)
    os.makedirs(os.path.join(HERE, 'classifications'), exist_ok=True)
    n = 0
    for fn in sorted(os.listdir(BUNDLES)):
        ...
        with open(os.path.join(HERE, 'data', 'prompts', fn[:-5] + '.txt'), 'w') as f:
